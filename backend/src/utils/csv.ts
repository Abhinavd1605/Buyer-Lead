...
import { 
  ...
  StatusSchema,
  validateBudget,
  validateBHKRequirement
} from '../validators';

// Enum mappings for CSV import
const CITY_MAPPING: Record<string, City> = {
...
const BHK_MAPPING: Record<string, BHK> = {
  'studio': 'STUDIO',
  '1': 'ONE',
  '2': 'TWO',
  '3': 'THREE',
...
const PURPOSE_MAPPING: Record<string, Purpose> = {
  'buy': 'BUY',
  'rent': 'RENT'
};

const TIMELINE_MAPPING: Record<string, Timeline> = {
...
const SOURCE_MAPPING: Record<string, Source> = {
  'website': 'WEBSITE',
  'referral': 'REFERRAL',
  'walk-in': 'WALK_IN',
  'call': 'CALL',
...
export const parseCSV = (csvBuffer: Buffer): Promise<CSVImportRow[]> => {
...
export const validateCSVRow = (row: CSVImportRow, rowIndex: number): { 
  isValid: boolean; 
  ...
  const transformedData: Partial<BuyerCreateInput> = {};
  ...
  if (transformedData.budgetMin && transformedData.budgetMax) {
    if (!validateBudget(transformedData.budgetMin, transformedData.budgetMax)) {
  ...
  return { 
    isValid, 
    errors, 
    data: isValid ? transformedData as BuyerCreateInput : undefined 
  };
};

export const generateCSV = async (data: any[], filename: string): Promise<string> => {
